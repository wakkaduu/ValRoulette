...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setAllAgents(json.data.filter((a: any, i: number, s: any[]) => 
  ...
  const skipBoot = () => {
    bootRadar.setValue(1);
    podAnims.forEach(anim => anim.setValue(1));
  };
  ...
  const generateSquad = () => {
    ...
    let pool = [...allAgents].sort(() => 0.5 - Math.random());
    let final: any[] = [];
    if (isBalanced) {
      ['Controller', 'Sentinel', 'Duelist', 'Initiator'].forEach(role => {
        const found = pool.find(a => a.role?.displayName === role);
        if (found) { final.push(found); pool = pool.filter(a => a.uuid !== found.uuid); }
      });
    }
    while (final.length < 5) final.push(pool.pop());
    final = final.sort(() => 0.5 - Math.random());
    setSquad(playerNames.map((name, i) => ({ ign: name || `P${i+1}`, agent: final[i] })));
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerArea}>
        ...
        <TouchableOpacity style={styles.iconBtn} onPress={() => setShowPresets(true)}>
          <Text style={styles.iconBtnText}>📂</Text>
        </TouchableOpacity>
      </View>

      <View style={styles.centerStage}>
        <TouchableOpacity style={styles.coreTouch} onPress={generateSquad} activeOpacity={0.8}>
          <Animated.View style={[styles.coreGlow, { backgroundColor: themeColor + '22', transform: [{ scale: pulseAnim }] }]} />
          <View style={[styles.coreButton, { backgroundColor: themeColor }]}><Text style={styles.coreText}>ROLL</Text></View>
        </TouchableOpacity>
        {playerNames.map((name, i) => {
          ...
          return (
            <Animated.View key={i} style={[styles.playerPod, { transform: [{ translateX: pos.x }, { translateY: pos.y }, { scale: podAnims[i] }], opacity: podAnims[i] }]}>
              <View style={styles.podHex}>
                {res ? <Image source={{ uri: res.agent.displayIcon }} style={styles.agentAvatar} /> : <View style={styles.emptyHex} />}
              </View>
              <TextInput style={styles.podInput} placeholder="IDENTIFY" placeholderTextColor="#3e4a56" value={name} onChangeText={(t) => { const n = [...playerNames]; n[i] = t; setPlayerNames(n); }} />
        ...
        <View style={styles.overlay}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.settingRow}>
              <Text style={styles.settingTxt}>ENABLE BOOT ANIMATIONS</Text>
              <Switch value={useAnimations} onValueChange={setUseAnimations} trackColor={{ false: "#161d24", true: themeColor }} />
            </View>

            <TouchableOpacity 
                style={[styles.aboutBtn, { borderColor: themeColor }]} 
                onPress={() => setExpandAbout(!expandAbout)}
            >
              <Text style={[styles.aboutBtnText, { color: themeColor }]}>
                {expandAbout ? "ABOUT" : "ABOUT"}
              </Text>
            </TouchableOpacity>
            ...
            {expandAbout && (
              <View style={styles.aboutContainer}>
                ...
                <Text style={styles.aboutDesc}>
                  Valorant Roulette is a performance-driven squad management interface 
                  designed for competitive 5-stacks. 
                </Text>
                <Text style={[styles.aboutTitle, { color: themeColor, marginTop: 20 }]}>CORE DEVELOPER</Text>
                <View style={styles.devRow}>
                  <View style={[styles.devIndicator, { backgroundColor: themeColor }]} />
                  <View style={{flex: 1}}>
                    <Text style={styles.devName}>IT Student</Text>
                    <Text style={styles.devSchool}>Quezon City University (QCU)</Text>
                    <Text style={[styles.devInfo, { color: themeColor }]}>21 y/o Developer • Created for fun!</Text>
                  </View>
                </View>
              </View>
            )}
            ...
            <View style={styles.versionInfo}><Text style={styles.versionTxt}>V1.9.0 // wakkadu</Text></View>
            <View style={{height: 100}} />
          </ScrollView>
        </View>
      </Modal>

      <Modal visible={showPresets} animationType="fade" transparent={true}>
        <View style={styles.overlay}>
          <View style={styles.modalHeader}>
